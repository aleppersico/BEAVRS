...
class Plots(object):
  ...
  def _set_colors(self):
    ...
    self.colors_mat = {
      ...
      self.mats['Air-U235']:           [255, 192, 203],   # RR1:        black
      self.mats['Borated Water-U235']: [255, 192, 203],   # RR1:        black
    }
  ...
  def _add_plots(self):
    ...
    # vox_plot.type = 'voxel'
    # # vox_plot.origin = (0, 0, 0)
    # # vox_plot.width = (482.6, 482.6, 420.0)
    
    # vox_plot.origin = [0, 0, H/2]
    # vox_plot.width = [H, H, H]

    # vox_plot.pixels = (1000, 1000, 870)
    # vox_plot.color_by = 'material'
    # vox_plot.filename = 'geometry_voxel'
    # self.plots.append(vox_plot)

    plot = openmc.Plot()
    plot.filename = 'central_FA_midplane_mat'
    plot.color_by = 'material'
    plot.basis = 'xy'
    plot.origin = [0, 0, H/2]
    plot.width = [c.latticePitch*2, c.latticePitch*2]
    plot.pixels = [2*res, 2*res]
    plot.colors = self.colors_mat
    self.plots.append(plot)
    
    plot_cell = openmc.Plot()
    plot_cell.filename = 'central_FA_midplane_cell'
    plot_cell.color_by = 'cell'
    plot_cell.basis = plot.basis
    plot_cell.origin = plot.origin
    plot_cell.width = plot.width
    plot_cell.pixels = plot.pixels
    # self.plots.append(plot_cell)